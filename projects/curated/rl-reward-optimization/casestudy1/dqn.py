...
class Args:
    ...
    """如果启用，torch.backends.cudnn.deterministic=False"""
    cuda: bool = True
    """如果启用，将默认使用CUDA"""
    track: bool = False
    """如果启用，此实验将通过Weights and Biases进行跟踪"""
    ...
    """是否保存模型到runs/{run_name}文件夹"""
    upload_model: bool = False
    """是否上传保存的模型到huggingface"""
    hf_entity: str = ""
    """模型仓库的Hugging Face Hub用户名或组织名"""
    ...
    """更新目标网络的时间步间隔"""
    batch_size: int = 256
    """从重放内存中采样的批量大小（增大以利用GPU并行）"""
    start_e: float = 1
    """起始epsilon（探索率）"""
    end_e: float = 0.05
    """结束epsilon（探索率）"""
    exploration_fraction: float = 0.5
    """从start_e到end_e所需的时间步占总时间步的比例"""
    learning_starts: int = 5000
    """开始学习的时间步（减少以更早开始训练）"""
    train_frequency: int = 4
    """训练频率（每4步训练一次，减少训练次数）"""

    reward_frequency: int = 2000
    """奖励更新的频率（增大以减少reward计算开销）"""
    hidden_dim: int = 256
    """网络中隐藏单元的数量"""
    encode_dim: int = 64
    """编码表示的维度"""
...
"""
    ...
    args = tyro.cli(Args)
    assert args.num_envs == 1, "暂不支持向量化环境"
    ...
    print(f"\n{'='*60}")
    print(f"开始训练: {args.env_id} | DQN | 种子={args.seed}")
    print(f"{'='*60}")
    print(f"使用设备: {device}")
    ...
    print(f"探索率: {args.start_e} -> {args.end_e}")
    print(f"{'='*60}\n")
            ...
            for info in infos["final_info"]:
                if info and "episode" in info:
                    ...
                    if episode_count % 5 == 0:
                        ...
                        print(f"步数={global_step:,} | 回合奖励={info['episode']['r'][0]:.1f} | 回合长度={info['episode']['l'][0]} | 速度={sps}步/秒")
        ...
        if global_step > args.learning_starts:
            if global_step % args.train_frequency == 0:
                ...
                if global_step % 100 == 0:
                    ...
                    sps = int(global_step / (time.time() - start_time))
                    print(f"  -> 速度: {sps} 步/秒 | TD损失: {loss.item():.4f} | Q值: {old_val.mean().item():.4f}")
