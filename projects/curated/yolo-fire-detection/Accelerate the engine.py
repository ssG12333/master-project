...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    print(label,"中心坐标：("+str((p2[0]-p1[0])/2+p1[0])+","+str((p2[1]-p1[1])/2+p1[1])+ ")")
    cv2.rectangle(img ,p1, p2 ,color, thickness=tl, lineType=cv2.LINE_AA)
    x_c=int((p2[0]-p1[0])/2+p1[0]) 
    y_c=int((p2[1]-p1[1])/2+p1[1])
    print(x_c,"xy_c")
    print(y_c,"xy_c")
    #print(xy_c,"xy_c")
    xy_c=[x_c,y_c]  
    #串口通信
    #uart.write(bytearray([255,255]))   #start with FF FF
    #uart.write(bytearray([252]))       #FC
...
if __name__ == "__main__":
    ...
    engine_file_path = "build/best_nano_v5.engine"
